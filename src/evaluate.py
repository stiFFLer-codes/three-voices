...
ASSESSMENT: list[tuple[str, str, dict[str, tuple[str, str]]]] = [
    ("Perceivable", "1.1.1 Text alternative for non-text content", {
        ...
        "ASHA": ("Pass",
            "`results/tables/tier_asha_<case>.txt` reproduces the card verbatim — the band label, the "
            "driver list (empty on a routine card, as on the PNG), next step, accountability line, "
            "disclaimer — so nothing on the PNG is image-only."),
        "mother": ("Partial",
    ...
    ("Perceivable", "1.4.1 Use of colour (colour not the only cue)", {
        ...
        "ASHA": ("Pass",
            "The header text states the level itself — 'LOW — routine care', 'ELEVATED — needs "
            "follow-up', 'HIGH — needs follow-up' — one distinct string per band, asserted distinct by "
            "a self-check in render.py. Hue restates it and nothing more; render the card in greyscale "
    ...
    ("Understandable", "3.2.4 Consistent identification", {
        ...
        "ASHA": ("Pass",
            ...
            "in both, confident_low GREEN in both. The card's header names the level in words, so the "
            "shared identity is legible and not merely chromatic."),
        "mother": ("Pass",
    ...
    ("Nielsen", "1. Visibility of system status", {
        ...
        "ASHA": ("Pass",
            "Since banding was unified, the card inherits the mother tier's uncertainty rule and states "
            "the result of it in words: confident_high (margin 1.000) heads 'HIGH — needs follow-up' "
            "while boundary_mid (margin 0.012) heads 'ELEVATED — needs follow-up', so the person making "
            "the referral can tell a confident call from a knife-edge one. The margin itself is still "
            "not printed — deliberately, since the card is the tier that must not turn into a "
    ...
    ("Nielsen", "2. Match between system and the real world", {
        ...
        "ASHA": ("Pass",
            ...
            "IQR is 0 on the clean set, so 102F is flagged as raised, which is the right behaviour. Age "
            "carries its own direction pair — failure_mode reads 'young age' for a 13-year-old, not "
            "'low age', which is not something a health worker would say."),
        "mother": ("Pass",
...
CORRECTED = [
    ...
    ("One band across both tiers",
     ...
     "identities.",
     "band_for() is computed once per case and passed to both tiers, and each band carries its own "
     "header wording ('LOW — routine care' / 'ELEVATED — needs follow-up' / 'HIGH — needs follow-up'), "
     "so the three states stay distinguishable without colour."),
